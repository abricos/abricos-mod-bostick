	...
	StickWidget.prototype = {
		init: function(data){
			...
			E.on(el, 'dblclick', function(e){
				var el = E.getTarget(e);
				if (__self.onDblClick(el)){ E.preventDefault(e); }
			});
			this.element = el;
		...
		onClick: function(el){
			...
			switch(el.id){
			case tp['bremove']: this.showRemoveDialog(); return true;
			}
			return false;
		},
		onDblClick: function(el){
			...
			case tp['id']:
			case tp['bd']:
			case tp['view']: this.setEditMode(); return true;
			}
	...
	StickManager.prototype = {
		init: function(callback){
			...
			E.on(document.body, 'mouseup', function(e){
				var el = E.getTarget(e);
				if (__self.onMouseUp(el, e)){ E.preventDefault(e); }
			});
			E.on(document.body, 'mousedown', function(e){
				var el = E.getTarget(e);
				if (__self.onMouseDown(el, e)){ E.preventDefault(e); }
			});
			E.on(document.body, 'mousemove', function(e){
				var el = E.getTarget(e);
				if (__self.onMouseMove(el, e)){ E.preventDefault(e); }
			});
		...
		onMouseMove: function(el, e){
			...
			if (L.isNull(stick)){ return false; }
			this._lastXY = xy;
		...
		moveTop: function(stick){
			if (!stick || L.isNull(stick)){ return; }
